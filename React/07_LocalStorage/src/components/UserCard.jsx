  ...
  return (
    <div className="p-4 border-1 border-white rounded flex flex-col gap-2 bg-gray-950 text-white">
      ...
      <div className="flex justify-between w-full gap-4">
        <button  className="bg-yellow-700 text-white py-1 px-2 rounded" 
        onClick={() => {
          setUpdatedData(users)
          setToggle((prev) => !prev)
        }}>Update</button>
        <button onClick={() => deleteUser(users.id)} className="bg-red-700 text-white py-1 px-2 rounded">Delete</button>
      </div>
    </div>
  );
};
